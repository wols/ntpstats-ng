...
Die von xref:Logstash.adoc[Logstash] in Echtzeit verarbeiteten Statistiken werden in Indizes mit dem Namensmuster `ntpstats-live-YYYY-MM-dd` - z. B. `ntpstats-live-{docdate}` - abgelegt.
...
Die bereits mit xref:Logstash.adoc[Logstash] erfolgte Typisierung wird innerhalb des Index beibehalten.footnote:[link:https://www.elastic.co/blog/index-vs-type[Elasticsearch - Index vs. Type, window="_blank"]]
